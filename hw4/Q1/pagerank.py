...
"""
def author():                                                                                             
        return 'alu64' # replace sroutray3 with your Georgia Tech username.                                                                                             
...
#        print("Max node id : {}".format(self.max_node_id))
        print("Max node id : {}, max source id : {}".format(self.max_node_id, len(self.node_degree)))
...
### Implement your code here
#############################################
                if  new_scores[target] == 0:
                    new_scores[target] = (1-damping_factor)*node_weights[target]
                new_scores[target] += damping_factor*scores[source]/self.node_degree[source]
            
            for i in range(self.max_node_id):
                if new_scores[i] == 0: new_scores[i] = (1-damping_factor)*node_weights[i]
            scores = new_scores
#############################################
